...
description: 放在任何工程目录根下，作为通向 AI_MEMORY 集中上下文的桥梁。
             同时服务于 AGENTS.md（Codex / Claude Code / 其他支持 AGENTS.md 的工具）。
             CLAUDE.md 应创建为指向 AGENTS.md 的符号链接，使不同 AI 工具读取同一份文件、保持同步。
---

<!-- AI_MEMORY_ROOT = ${AI_MEMORY_ROOT} -->
<!-- 推荐先在系统/用户环境变量中设置 AI_MEMORY_ROOT。
     如果当前 AI 工具无法读取环境变量，可将上一行改为显式绝对路径。
     支持 ${AI_MEMORY_ROOT} / $AI_MEMORY_ROOT / %AI_MEMORY_ROOT% / env:AI_MEMORY_ROOT -->
...
AI_MEMORY 是集中式 AI 协作上下文仓库。本 bridge 文件让当前项目通过环境变量 `AI_MEMORY_ROOT` 引用它。

### AI_MEMORY_ROOT

推荐在每台机器上设置一次系统/用户环境变量，然后所有项目的 bridge 都保持：
...
```
# 小项目 / 临时 → 只有 workflow，不碰记忆
GLOBAL_WORKFLOW_RO = on，其余 off

# 中型项目 → 读项目记忆，只读不写
GLOBAL_WORKFLOW_RO + PROJECT_MEMORY_RO + SKILLS_RO = on
